...
table_sample::table_sample(prod *p) : table_ref(p)
{
    ...
    percent = 0.1 * d100();
    method = (d6() > 2) ? "system" : "bernoulli";  // 伯努利分布
}
...
simple_join_cond::simple_join_cond(prod *p, table_ref &lhs, table_ref &rhs)   // 等值连接
     : join_cond(p, lhs, rhs)
{
retry:
...
// show语句
show_stmt::show_stmt(prod *p) : prod(p)
{
...
alter_stmt::alter_stmt(prod *p, struct scope *s, table *v)
    : modifying_stmt(p, s, v)
{
    scope->refs.push_back(victim);
}

void alter_stmt::out(std::ostream &out)
{
    out << "alter table " << victim->ident()
        << " rename to " << victim->ident();
}

desc_stmt::desc_stmt(prod *p, struct scope *s, table *v)
    : modifying_stmt(p, s, v)
{
    scope->refs.push_back(victim);
}

void desc_stmt::out(std::ostream &out)
{
    out << "desc " << victim->ident();
}
...
//      return make_shared<merge_stmt>((struct prod *)0, s);
    ...
    else if (d42() > 5 && d42() < 10)
    {
        return make_shared<delete_returning>((struct prod *)0, s);
    }
    else if (d42() > 10 && d42() < 15)
    {
      return make_shared<upsert_stmt>((struct prod *)0, s);
    }
    else if (d42() > 15 && d42() < 20)
    {
        return make_shared<update_returning>((struct prod *)0, s);
    }
    // alter语句
    else if (d42() > 20 && d42() < 25)
    {
        return make_shared<alter_stmt>((struct prod *)0, s);
    }
    // show 语句
    else if (d42() > 25 && d42() < 30)
    {
        return make_shared<show_stmt>((struct prod *)0);
    }
    // desc 语句
    else if (d42() > 30 && d42() < 35)
    {
        return make_shared<desc_stmt>((struct prod *)0, s);
    }
//    else if (d6() > 4)
